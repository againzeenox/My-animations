...
######################################################
        ...
        black_rectangle = Rectangle(
            ...
            1
        ) #This Rectangle will fill into the entire screen
        ...
        RD = SVGMobject(
            f"{home}\\reddit-logo-2436.svg").shift(
                2 * RIGHT
            ) #Reddit SVG
...
######################################################
        ...
        ) #This function will create a Sinusoid that will be dependent on the value of Amplitude and Period
        ...
        equation = Tex(
            ...
            YELLOW
        ) #This is just the label of the above equation

        AmplitudeLine = always_redraw(
            lambda : Line(
                start = axis.c2p(
                    0, Amplitude.get_value(
        ...
        ) #This line will start from (0, Amplitude(x)) and will end at (Period(x), Amplitude(x))
        ...
        ) #This line will start from (Period(x), Amplitude(x)) and will end at (Period(x), 0)

        AmplitudeValue = always_redraw(
        ...
        AmplitudeBrace = always_redraw(
            lambda : BraceBetweenPoints(
                AmplitudeLine.get_end(

                ), AmplitudeLine.get_start(

                )
            )
        ) #This is a brace around the Amplitude Line

        AmplitudeTex = always_redraw(
            lambda : AmplitudeBrace.get_text(
                "Period"
            ).set_color(
                TEAL
            )
        ) #Text for AmplitudeBrace

        PeriodBrace = always_redraw(
            lambda : BraceBetweenPoints(
                PeriodLine.get_end(

                ), PeriodLine.get_start(

                )
            )
        ) #This is a brace around the PeriodLine

        PeriodTex = always_redraw(
            lambda : PeriodBrace.get_text(
                "Amplitude"
            ).set_color(
                PINK
            )
        ) #Text for PeriodBrace
        ...
        IndependentAmplitude = Tex(
            ...
            RED, BLUE, PURPLE_C, GREEN_B
        ).shift(
            DOWN
        ) #It's just the text about Amplitude
        ...
        IndependentPeriod = Tex(
            ...
            DOWN
        ) #It's just the text about Period

        SurroundingRectangleForAmplitude = SurroundingRectangle(
            AmplitudeValue
        )

        SurroundingRectangleForPeriod = SurroundingRectangle(
            PeriodValue
        )
...
######################################################
        ...
        self.play(
            Write(
                VGroup(
                    equation, AmplitudeEqual, AmplitudeValue, PeriodEqual, PeriodValue
                )
            )
        )
        ...
        self.play(
            Write(
                VGroup(
                    AmplitudeTex, PeriodTex
                )
            )
        )
        
        self.play(
            Amplitude.animate.set_value(
                -10
            ), Period.animate.set_value(
                -10
            ), axis.animate.scale(
                1/5
            ), lag_ratio = 0
        ...
        self.play(
            TransformMatchingShapes(
                IndependentPeriod, IndependentAmplitude
            ),  Indicate(
                AmplitudeLine
            ), Flash(
                AmplitudeLine
            ), Create(
                SurroundingRectangleForAmplitude
            )
        )
        ...
        ) #This animation will have an effect of every mobject on screen "Fading Out"
        ...
        self.play(
            Write(
                VGroup(
                    YT_name, IG_name, RD_name, DS_name
                )
            )
        ) #Reach out names
